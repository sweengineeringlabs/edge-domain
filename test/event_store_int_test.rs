...
use edge_domain::{
    new_in_memory_event_store, reconstitute, Aggregate, DomainEvent, EventStoreError,
    ExpectedVersion,
};
...
async fn test_append_returns_new_stream_version() {
    let store = new_in_memory_event_store::<CounterIncremented>();
    let ver = store
        .append(
            "c1",
...
async fn test_reconstitute_returns_none_for_unknown_aggregate() {
    let store = new_in_memory_event_store::<CounterIncremented>();
    let result = reconstitute::<Counter>(&*store, "missing")
        .await
        .expect("reconstitute");
...
async fn test_reconstitute_rebuilds_aggregate_from_events() {
    ...
    let counter = reconstitute::<Counter>(&*store, "c1")
        .await
        .expect("reconstitute")
        .expect("must exist");
...
async fn test_reconstitute_is_idempotent() {
    ...
    let a = reconstitute::<Counter>(&*store, "c1")
        .await
        .expect("first")
        .expect("some");
    let b = reconstitute::<Counter>(&*store, "c1")
        .await
        .expect("second")
        .expect("some");
