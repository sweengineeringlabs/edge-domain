...
struct Product { name: String, price_cents: u32 }

struct ExpensiveProducts { threshold: u32 }
impl Spec<Product> for ExpensiveProducts {
    fn matches(&self, p: &Product) -> bool { p.price_cents >= self.threshold }
}

struct NameStartsWith(String);
impl Spec<Product> for NameStartsWith {
    fn matches(&self, p: &Product) -> bool { p.name.starts_with(&self.0) }
}
...
async fn seeded_repo() -> Arc<dyn QueryableRepository<Product, u32>> {
    ...
    repo.save(4, Product { name: "Blueberry".into(), price_cents: 500 }).await.unwrap();
    repo
}
...
async fn test_queryable_repository_find_by_returns_matching_entities() {
    ...
    let results = repo.find_by(&ExpensiveProducts { threshold: 200 }).await.unwrap();
...
async fn test_queryable_repository_find_by_returns_empty_when_none_match() {
    let repo = seeded_repo().await;
    let results = repo.find_by(&ExpensiveProducts { threshold: 9999 }).await.unwrap();
...
async fn test_queryable_repository_find_one_by_returns_first_match() {
    let repo = seeded_repo().await;
    let result = repo.find_one_by(&ExpensiveProducts { threshold: 400 }).await.unwrap();
...
async fn test_queryable_repository_find_one_by_returns_none_when_no_match() {
    let repo = seeded_repo().await;
    let result = repo.find_one_by(&ExpensiveProducts { threshold: 9999 }).await.unwrap();
...
async fn test_queryable_repository_count_by_returns_correct_count() {
    let repo = seeded_repo().await;
    let count = repo.count_by(&ExpensiveProducts { threshold: 200 }).await.unwrap();
...
async fn test_queryable_repository_count_by_returns_zero_when_no_match() {
    let repo = seeded_repo().await;
    let count = repo.count_by(&ExpensiveProducts { threshold: 9999 }).await.unwrap();
    assert_eq!(count, 0);
}
