...
use edge_llm_prompt::{
    PromptBootstrap, PromptTemplate, StdPromptFactory, TemplateProvider, TEMPLATE_PROVIDER_SVC,
};
...
fn test_get_template_registered_id_happy() {
    let p = provider();
    assert_eq!(
        p.get_template("code-review").expect("present").id,
        "code-review"
    );
}
...
fn test_get_template_empty_registry_edge() {
    ...
    assert!(empty.get_template("code-review").is_none());
}
...
fn test_list_templates_returns_all_happy() {
    assert_eq!(provider().list_templates().len(), 3);
}
...
fn test_list_templates_empty_registry_error() {
    let empty = StdPromptFactory::template_provider();
    assert!(empty.list_templates().is_empty());
}
...
fn test_list_templates_duplicate_id_collapses_edge() {
    ...
    assert_eq!(p.list_templates().len(), 1);
}
...
fn test_list_by_category_matching_happy() {
    let p = provider();
    let code = p.list_by_category("code");
...
fn test_list_by_category_empty_registry_edge() {
    let empty = StdPromptFactory::template_provider();
    assert!(empty.list_by_category("code").is_empty());
}
