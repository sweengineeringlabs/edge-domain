//! `CompleteFactory` — constructor contract for the default complete primitives.
...
pub trait CompleteFactory {
    ...
    fn tool_message(content: String, tool_call_id: String) -> Message {
    ...
    fn image_url(url: String) -> ImageUrl {
        ImageUrl::new(url)
    }

    /// Build an image-URL [`ContentPart`] from an [`ImageUrl`].
    fn image_part(image_url: ImageUrl) -> ContentPart {
        ContentPart::image_url(image_url)
    }

    /// Build a [`ToolDefinition`].
    fn tool_definition(name: String, description: String, parameters: Value) -> ToolDefinition {
        ToolDefinition::new(name, description, parameters)
    }

    /// Build a [`ToolCall`].
    fn tool_call(id: String, name: String, arguments: String) -> ToolCall {
        ToolCall::new(id, name, arguments)
    }

    /// Build an initial [`ToolCallDelta`] at the given stream index.
    fn tool_call_delta(index: u32) -> ToolCallDelta {
        ToolCallDelta::new(index)
    }

    /// Build a [`StreamDelta`] carrying text content.
    fn stream_delta(content: String) -> StreamDelta {
        StreamDelta::text(content)
    }

    /// Build a terminal [`StreamChunk`] with a finish reason.
    fn stream_chunk(id: String, delta: StreamDelta, finish_reason: FinishReason) -> StreamChunk {
    ...
    fn tool_choice_auto() -> ToolChoice {
        ToolChoice::Auto
    }

    /// Return the standard factory implementation.
    fn std_complete_factory() -> StdCompleteFactory {
        StdCompleteFactory
    }
}
