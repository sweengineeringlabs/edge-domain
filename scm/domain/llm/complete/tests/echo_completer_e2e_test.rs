...
use edge_llm_complete::{
    CompleteError, CompleteRequest, Completer, CompletionRequest, EchoCompleter, Message,
    ModelInfoRequest, ProcessingRequest, Processor, SupportedModelsRequest, ValidationRequest,
    Validator,
};
...
fn test_echo_completer_model_info_unknown_returns_error_error() {
    let err =
        block_on(EchoCompleter.model_info(ModelInfoRequest { model: "unknown" })).unwrap_err();
...
fn test_echo_completer_validator_accepts_valid_request_edge() {
    ...
    assert!(matches!(
        EchoCompleter.validate(ValidationRequest { request: &req }),
        Ok(())
    ));
}
