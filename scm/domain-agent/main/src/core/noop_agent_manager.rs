...
/// A no-op agent manager that implements the AgentManager trait but performs no work.
/// Used for testing the contract; real implementations live in plugins.
pub(crate) struct NoopAgentManager;
...
mod tests {
    ...
    fn test_noop_agent_manager_happy_list_agent_ids_returns_empty() {
        assert_eq!(NoopAgentManager.list_agent_ids().unwrap().len(), 0);
    }
