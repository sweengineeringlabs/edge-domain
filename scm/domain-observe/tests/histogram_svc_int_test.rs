use edge_domain_observe::HISTOGRAM_SVC;
...
fn test_histogram_svc_key_has_edge_prefix_error() {
    assert!(HISTOGRAM_SVC.starts_with("edge."), "service key must be namespaced under edge.*");
}

#[test]
fn test_histogram_svc_key_stable_across_calls_edge() {
    assert_eq!(HISTOGRAM_SVC, HISTOGRAM_SVC);
}
