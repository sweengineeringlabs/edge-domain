...
use edge_domain_security::{CREDENTIAL_RESOLVER_SVC, CREDENTIAL_RESOLVER_SVC_FACTORY};
...
fn test_credential_resolver_svc_error() {
    assert!(
        !CREDENTIAL_RESOLVER_SVC.is_empty(),
...
fn test_credential_resolver_svc_edge() {
    assert_eq!(
        CREDENTIAL_RESOLVER_SVC.len(),
        "credential_resolver".len(),
