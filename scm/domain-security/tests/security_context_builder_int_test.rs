...
fn test_builder_via_bootstrap_happy() {
    ...
    assert!(!ctx.authenticated, "builder must create unauthenticated context");
}
...
fn test_builder_via_bootstrap_edge() {
    ...
    assert_eq!(ctx1.authenticated, ctx2.authenticated, "multiple builders must produce consistent state");
}
