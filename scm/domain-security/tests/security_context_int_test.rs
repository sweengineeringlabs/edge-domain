...
fn test_security_context_unauthenticated_happy() {
    ...
    assert!(!ctx.authenticated, "Unauthenticated context must have authenticated=false");
}

#[test]
fn test_security_context_unauthenticated_error() {
    let ctx = <edge_domain_security::SecurityServices as SecurityBootstrap>::unauthenticated();
    assert!(ctx.principal.is_none(), "Unauthenticated context must have no principal");
}

#[test]
fn test_security_context_unauthenticated_edge() {
    let ctx = <edge_domain_security::SecurityServices as SecurityBootstrap>::unauthenticated();
    assert!(ctx.claims.is_empty(), "Unauthenticated context must have no claims");
}

#[test]
fn test_security_context_authenticated_happy() {
    let principal = Box::new(edge_domain_security::AnonymousPrincipal);
    let ctx = <edge_domain_security::SecurityServices as SecurityBootstrap>::authenticated(principal);
    assert!(ctx.authenticated, "Authenticated context must have authenticated=true");
}
...
fn test_security_context_authenticated_error() {
    let principal = Box::new(edge_domain_security::AnonymousPrincipal);
    let ctx = <edge_domain_security::SecurityServices as SecurityBootstrap>::authenticated(principal);
    assert!(ctx.principal.is_some(), "Authenticated context must have a principal");
...
fn test_security_context_debug_happy() {
    ...
    assert!(debug_str.contains("false"), "Debug must show authenticated state");
}
