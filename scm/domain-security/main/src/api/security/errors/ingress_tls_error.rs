//! [`IngressTlsError`] — TLS configuration and handshake errors.
...
pub enum IngressTlsError {
    ...
    /// Certificate PEM data could not be parsed, or the file contained no certificates.
    ...
    /// Private key PEM data could not be parsed, or the file contained no key.
    #[error("failed to parse private key: {0}")]
    KeyParse(String),

    /// The TLS server configuration could not be built from the provided certificate and key.
