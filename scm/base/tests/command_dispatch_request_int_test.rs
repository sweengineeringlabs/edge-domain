...
use edge_application_base::{Command, CommandDispatchRequest, CommandError, CommandExecutionRequest};
...
fn test_command_dispatch_request_wraps_the_given_command_happy() {
    let req = CommandDispatchRequest {
        command: Box::new(Ok_),
    };
    assert_eq!(block_on(req.command.execute(CommandExecutionRequest)), Ok(()));
}
...
fn test_command_dispatch_request_preserves_a_failing_commands_error_error() {
    ...
    assert!(block_on(req.command.execute(CommandExecutionRequest)).is_err());
}
