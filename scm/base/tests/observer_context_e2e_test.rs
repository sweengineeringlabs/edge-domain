...
impl ObserverContext for FakeContext {
    fn tracer(
        &self,
        _req: TracerRequest,
    ) -> Result<edge_application_base::TracerResponse<'_>, ObserveError> {
        Ok(edge_application_base::TracerResponse { tracer: &self.tracer })
    }
    fn drain(
        &self,
        _req: DrainRequest,
    ) -> Result<edge_application_base::DrainResponse<'_>, ObserveError> {
        Ok(edge_application_base::DrainResponse { drain: &self.drain })
    }
    fn metrics(
        &self,
        _req: MetricsRequest,
    ) -> Result<edge_application_base::MetricsResponse<'_>, ObserveError> {
        Ok(edge_application_base::MetricsResponse {
