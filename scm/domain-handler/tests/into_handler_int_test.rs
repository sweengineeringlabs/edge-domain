...
use edge_domain_handler::{Handler, HandlerContext, IntoHandler, Validator, INTO_HANDLER_SVC};
...
fn test_into_handler_empty_name_fails_validation_error() {
    let h = EmptyNameService.into_handler();
...
async fn test_into_handler_executes_ok_service_happy() {
    let handler = EchoService.into_handler();
    let security = SecurityContext::unauthenticated();
    let observer = StdObserveFactory::noop_observer_context();
    let ctx = make_ctx(&security, observer.as_ref());
    assert_eq!(handler.execute("hello".into(), ctx).await.unwrap(), "hello");
}

/// @covers: IntoHandler::into_handler
#[tokio::test]
async fn test_into_handler_propagates_service_error() {
    let handler = FailingService.into_handler();
    let security = SecurityContext::unauthenticated();
    let observer = StdObserveFactory::noop_observer_context();
    let ctx = make_ctx(&security, observer.as_ref());
    let err = handler.execute("x".into(), ctx).await.unwrap_err();
...
async fn test_into_handler_empty_name_service_executes_edge() {
    let handler = EmptyNameService.into_handler();
    let security = SecurityContext::unauthenticated();
    let observer = StdObserveFactory::noop_observer_context();
    let ctx = make_ctx(&security, observer.as_ref());
    assert_eq!(handler.execute("".into(), ctx).await.unwrap(), "");
}
