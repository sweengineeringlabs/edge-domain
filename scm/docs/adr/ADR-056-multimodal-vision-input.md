...
- **Does not touch the `supports_vision`-is-a-plain-`bool` shape question** raised in GitHub issue #83 (whether model capabilities should be a `ModelCapability` enum set). This ADR reads the existing `bool` as-is; the type-shape question is orthogonal and tracked separately.
...
- `ModelInfo.supports_vision` becomes load-bearing: previously-dead data (zero real consumers, confirmed by grep) now gates a real code path and returns a typed, structured `ExecutionError::VisionNotSupported { model }` instead of silently sending an image to a model that can't use it or silently dropping it.
