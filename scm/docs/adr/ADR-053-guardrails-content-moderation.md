...
- Today the only production `Completer` implementations in the workspace are `EchoCompleter`, `NoopCompleter`, and `EchoProviderCompleter` (confirmed via `grep -l "impl Completer for"` — 3 hits in non-test code; the same grep also matches 2 test-only `ScriptedCompleter` mocks in `agents/tests/conversation_loop_e2e_test.rs` and `complete/tests/tool_call_loop_e2e_test.rs`, neither doing content inspection either). None of the production implementations ever constructs `ContentFiltered`. ADR-048 (proposed same day) is what stands up the first real vendor `Completer` (`AnthropicCompleter`); even once that ships, `ContentFiltered` would only fire if Anthropic's own moderation rejects a request — still zero edge-side policy.
