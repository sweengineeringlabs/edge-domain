...
fn test_save_then_find_returns_entity_happy() {
    let repo = make();
    block_on(repo.save(1, "order-1".into())).unwrap_or_default();
    let found = block_on(repo.find(&1)).unwrap_or(None);
...
fn test_find_missing_id_returns_none_error() {
    let repo = make();
    let found = block_on(repo.find(&99)).unwrap_or(Some("x".into()));
...
fn test_save_existing_id_leaves_other_entries_intact_error() {
    let repo = make();
    block_on(repo.save(1, "original".into())).unwrap_or_default();
    block_on(repo.save(2, "other".into())).unwrap_or_default();
    block_on(repo.save(1, "updated".into())).unwrap_or_default();
    let other = block_on(repo.find(&2)).unwrap_or(None);
...
fn test_save_overwrites_existing_entry_edge() {
    let repo = make();
    block_on(repo.save(1, "first".into())).unwrap_or_default();
    block_on(repo.save(1, "second".into())).unwrap_or_default();
    let found = block_on(repo.find(&1)).unwrap_or(None);
...
fn test_delete_existing_entity_returns_true_happy() {
    let repo = make();
    block_on(repo.save(2, "item".into())).unwrap_or_default();
    let removed = block_on(repo.delete(&2)).unwrap_or(false);
    assert!(removed);
    let found = block_on(repo.find(&2)).unwrap_or(Some("x".into()));
    assert!(found.is_none());
}
...
fn test_delete_missing_entity_returns_false_error() {
    let repo = make();
    let removed = block_on(repo.delete(&42)).unwrap_or(true);
    assert!(!removed);
}
...
fn test_delete_already_deleted_returns_false_edge() {
    let repo = make();
    block_on(repo.save(3, "item".into())).unwrap_or_default();
    block_on(repo.delete(&3)).unwrap_or(false);
    let second = block_on(repo.delete(&3)).unwrap_or(true);
    assert!(!second);
}
...
fn test_exists_saved_entity_returns_true_happy() {
    let repo = make();
    block_on(repo.save(3, "y".into())).unwrap_or_default();
    let exists = block_on(repo.exists(&3)).unwrap_or(false);
    assert!(exists);
}
...
fn test_exists_missing_entity_returns_false_error() {
    let repo = make();
    let exists = block_on(repo.exists(&7)).unwrap_or(true);
    assert!(!exists);
}
...
fn test_exists_deleted_entity_returns_false_edge() {
    let repo = make();
    block_on(repo.save(5, "gone".into())).unwrap_or_default();
    block_on(repo.delete(&5)).unwrap_or(false);
    let exists = block_on(repo.exists(&5)).unwrap_or(true);
    assert!(!exists);
}
...
fn test_count_after_saves_returns_correct_count_happy() {
    ...
    block_on(repo.save(2, "b".into())).unwrap_or_default();
    let n = block_on(repo.count()).unwrap_or(0);
    assert_eq!(n, 2);
}
