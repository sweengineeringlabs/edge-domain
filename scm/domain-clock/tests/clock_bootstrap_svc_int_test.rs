...
use edge_domain_clock::{BootstrapNameRequest, Clock, ClockBootstrap, NowRequest, StdClockFactory};
...
fn test_bootstrap_name_is_idempotent_error() {
    ...
    assert_eq!(name1, "clock", "bootstrap_name must return expected identifier");
}
...
fn test_fixed_returns_frozen_clock_happy() {
    ...
    assert_eq!(
        ...
        instant,
        "Fixed clock must return the provided instant"
    );
}
...
fn test_fixed_with_epoch_boundary_edge() {
    ...
    assert_eq!(
        clock.now(NowRequest).unwrap().instant,
        instant,
        "Fixed clock must handle UNIX_EPOCH"
    );
}
