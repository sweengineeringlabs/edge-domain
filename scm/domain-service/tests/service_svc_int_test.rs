...
fn test_name_via_dyn_dispatch_returns_name_edge() {
    let svc: &dyn Service<Request = TextPayload, Response = TextPayload> = &Echo("ping".into());
...
fn test_execute_ok_service_returns_response_happy() {
    let result = block_on(Echo("echo".into()).execute(TextPayload("hello".into())));
    assert_eq!(result.ok().map(|r| r.0), Some("hello".to_string()));
}

/// @covers: Service::execute — failure propagates error
#[test]
fn test_execute_failing_service_returns_err_error() {
    let result = block_on(AlwaysFails.execute(TextPayload("any".into())));
...
fn test_execute_repeated_calls_are_independent_edge() {
    let svc = Echo("e".into());
    assert_eq!(
        block_on(svc.execute(TextPayload("a".into()))).ok().map(|r| r.0),
        Some("a".to_string())
    );
    assert_eq!(
        block_on(svc.execute(TextPayload("b".into()))).ok().map(|r| r.0),
        Some("b".to_string())
    );
}
