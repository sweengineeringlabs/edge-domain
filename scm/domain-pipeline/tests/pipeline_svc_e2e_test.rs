...
fn test_create_pipeline_with_config_happy_applies_config() {
    ...
    let pipeline = PipelineSvc::build(PipelineBuilder::<i32> { steps: vec![], config });
...
fn test_create_pipeline_with_config_happy_default_config() {
    let pipeline = PipelineSvc::build(PipelineBuilder::<i32> { steps: vec![], config: PipelineConfig::default() });
    let result_config = pipeline.config();
