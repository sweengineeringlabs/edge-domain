//! Integration tests for `abort_on_error` config — ADR-048 Phase 1.
#![allow(clippy::unwrap_used, clippy::expect_used)]

use edge_domain_pipeline::{
    ContextMutationRequest, PipelineBuilder, PipelineConfig, PipelineError, PipelineSvc, Step,
    StepNameRequest, StepNameResponse,
};

struct IncrementStep;

#[async_trait::async_trait]
impl Step for IncrementStep {
    type Ctx = Vec<i32>;
    type ExecutionError = String;

    async fn execute(&self, req: ContextMutationRequest<'_, Vec<i32>>) -> Result<(), String> {
        req.ctx.push(req.ctx.len() as i32);
        Ok(())
    }

    fn name(&self, _req: StepNameRequest) -> Result<StepNameResponse, PipelineError<String>> {
        Ok(StepNameResponse {
            name: "increment".to_string(),
        })
    }
}

struct FailStep;

#[async_trait::async_trait]
impl Step for FailStep {
    type Ctx = Vec<i32>;
    type ExecutionError = String;

    async fn execute(&self, _req: ContextMutationRequest<'_, Vec<i32>>) -> Result<(), String> {
        Err("fail".to_string())
    }

    fn name(&self, _req: StepNameRequest) -> Result<StepNameResponse, PipelineError<String>> {
        Ok(StepNameResponse {
            name: "fail".to_string(),
        })
    }
}

// ── abort_on_error = true (default) ──────────────────────────────────────────

#[tokio::test]
async fn test_abort_on_error_happy_stops_at_first_failure() {
    let pipeline: Box<
        dyn edge_domain_pipeline::Pipeline<
            Ctx = Vec<i32>,
            E = String,
            Request = Vec<i32>,
            Response = Vec<i32>,
        >,
    > = PipelineSvc::build(
        PipelineBuilder::new()
            .with(IncrementStep)
            .with(FailStep)
            .with(IncrementStep),
    );
    let mut ctx: Vec<i32> = vec![];
    let result = pipeline.run(ContextMutationRequest { ctx: &mut ctx }).await;
    assert!(result.is_err());
    assert_eq!(ctx.len(), 1, "only one step ran before abort");
}

#[tokio::test]
async fn test_abort_on_error_happy_propagates_error() {
    let pipeline: Box<
        dyn edge_domain_pipeline::Pipeline<
            Ctx = Vec<i32>,
            E = String,
            Request = Vec<i32>,
            Response = Vec<i32>,
        >,
    > = PipelineSvc::build(PipelineBuilder::new().with(FailStep));
    let mut ctx: Vec<i32> = vec![];
    match pipeline.run(ContextMutationRequest { ctx: &mut ctx }).await {
        Err(PipelineError::StepFailed(e)) => assert_eq!(e.cause, "fail"),
        other => panic!("expected StepFailed, got {:?}", other),
    }
}

#[tokio::test]
async fn test_abort_on_error_edge_succeeds_with_no_errors() {
    let pipeline: Box<
        dyn edge_domain_pipeline::Pipeline<
            Ctx = Vec<i32>,
            E = String,
            Request = Vec<i32>,
            Response = Vec<i32>,
        >,
    > = PipelineSvc::build(
        PipelineBuilder::new()
            .with(IncrementStep)
            .with(IncrementStep),
    );
    let mut ctx: Vec<i32> = vec![];
    assert!(pipeline
        .run(ContextMutationRequest { ctx: &mut ctx })
        .await
        .is_ok());
    assert_eq!(ctx.len(), 2);
}

// ── abort_on_error = false ────────────────────────────────────────────────────

#[tokio::test]
async fn test_abort_on_error_false_happy_continues_past_failure() {
    let pipeline: Box<
        dyn edge_domain_pipeline::Pipeline<
            Ctx = Vec<i32>,
            E = String,
            Request = Vec<i32>,
            Response = Vec<i32>,
        >,
    > = PipelineSvc::build(
        PipelineBuilder::new()
            .abort_on_error(false)
            .with(IncrementStep)
            .with(FailStep)
            .with(IncrementStep),
    );
    let mut ctx: Vec<i32> = vec![];
    let result = pipeline.run(ContextMutationRequest { ctx: &mut ctx }).await;
    assert!(
        result.is_ok(),
        "pipeline should succeed when abort_on_error=false"
    );
    assert_eq!(ctx.len(), 2, "both increment steps must have run");
}

#[tokio::test]
async fn test_abort_on_error_false_error_all_steps_run_despite_failures() {
    let pipeline: Box<
        dyn edge_domain_pipeline::Pipeline<
            Ctx = Vec<i32>,
            E = String,
            Request = Vec<i32>,
            Response = Vec<i32>,
        >,
    > = PipelineSvc::build(
        PipelineBuilder::new()
            .abort_on_error(false)
            .with(FailStep)
            .with(IncrementStep)
            .with(FailStep)
            .with(IncrementStep),
    );
    let mut ctx: Vec<i32> = vec![];
    assert!(pipeline
        .run(ContextMutationRequest { ctx: &mut ctx })
        .await
        .is_ok());
    assert_eq!(
        ctx.len(),
        2,
        "both increment steps must have run past failures"
    );
}

#[tokio::test]
async fn test_abort_on_error_false_edge_no_steps_returns_ok() {
    let pipeline: Box<
        dyn edge_domain_pipeline::Pipeline<
            Ctx = Vec<i32>,
            E = String,
            Request = Vec<i32>,
            Response = Vec<i32>,
        >,
    > = PipelineSvc::build(PipelineBuilder::<Vec<i32>, String>::new().abort_on_error(false));
    let mut ctx: Vec<i32> = vec![];
    assert!(pipeline
        .run(ContextMutationRequest { ctx: &mut ctx })
        .await
        .is_ok());
}

// ── via create_pipeline_with_config ──────────────────────────────────────────

#[tokio::test]
async fn test_abort_on_error_via_config_happy_stops_on_error() {
    use std::sync::Arc;
    let steps: Vec<Arc<dyn Step<Ctx = Vec<i32>, ExecutionError = String>>> = vec![
        Arc::new(IncrementStep),
        Arc::new(FailStep),
        Arc::new(IncrementStep),
    ];
    let config = PipelineConfig {
        abort_on_error: true,
        ..PipelineConfig::default()
    };
    let pipeline = PipelineSvc::build(PipelineBuilder {
        steps,
        config,
        event_bus: None,
    });
    let mut ctx: Vec<i32> = vec![];
    assert!(pipeline
        .run(ContextMutationRequest { ctx: &mut ctx })
        .await
        .is_err());
    assert_eq!(ctx.len(), 1);
}

#[tokio::test]
async fn test_abort_on_error_via_config_error_continues_when_false() {
    use std::sync::Arc;
    let steps: Vec<Arc<dyn Step<Ctx = Vec<i32>, ExecutionError = String>>> = vec![
        Arc::new(IncrementStep),
        Arc::new(FailStep),
        Arc::new(IncrementStep),
    ];
    let config = PipelineConfig {
        abort_on_error: false,
        ..PipelineConfig::default()
    };
    let pipeline = PipelineSvc::build(PipelineBuilder {
        steps,
        config,
        event_bus: None,
    });
    let mut ctx: Vec<i32> = vec![];
    assert!(pipeline
        .run(ContextMutationRequest { ctx: &mut ctx })
        .await
        .is_ok());
    assert_eq!(ctx.len(), 2);
}

#[tokio::test]
async fn test_abort_on_error_via_config_edge_all_pass() {
    use std::sync::Arc;
    let steps: Vec<Arc<dyn Step<Ctx = Vec<i32>, ExecutionError = String>>> =
        vec![Arc::new(IncrementStep), Arc::new(IncrementStep)];
    let config = PipelineConfig::default();
    let pipeline = PipelineSvc::build(PipelineBuilder {
        steps,
        config,
        event_bus: None,
    });
    let mut ctx: Vec<i32> = vec![];
    assert!(pipeline
        .run(ContextMutationRequest { ctx: &mut ctx })
        .await
        .is_ok());
    assert_eq!(ctx.len(), 2);
}
